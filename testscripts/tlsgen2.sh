...
# Issuer: O = Redis Labs, CN = RL Root Certificate Authority
# Subject: O = Redis Labs, CN = RL Root Certificate Authority
# Subject Public Key Info:
#     Public Key Algorithm: rsaEncryption
#         RSA Public-Key: (4096 bit)
# Signature Algorithm: sha256WithRSAEncryption
root_ca() {
    printf "** Creating Root Certificate Authority\n"
    
    printf "** Creating Root CA private key\n\n"
    openssl genrsa -out tls/root_key.pem 4096

    printf "** Creating Root CA public certificate\n\n"
    openssl req \
        -x509 -new -nodes -sha256 \
        -key tls/root_key.pem \
        -days 3650 \
        -subj '/O=Redis Labs/CN=RL Root Certificate Authority' \
        -out tls/root_cert.pem
}
...
intermediate_ca(){
    ...
    printf "** Creating Intermediate CA CSR\n\n"
    ...
    -in tls/ca.csr \
    -CA tls/root_cert.pem -CAkey tls/root_key.pem -CAserial tls/ca.txt -CAcreateserial \
    -out tls/ca_cert.pem
